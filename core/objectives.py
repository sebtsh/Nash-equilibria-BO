...
def sample_GP_prior(kernel, lowers, uppers, num_points, rng, jitter=1e-06):
    """
...
def sample_GP_prior_utilities(num_agents, kernel, lowers, uppers, num_points, rng):
    """
    ...
    """
    return [
        sample_GP_prior(kernel, lowers, uppers, num_points, rng)
...
def noisy_observer(u, noise_variance, rng):
    ...
    :param u: List of N Callables that each take in an array of shape (n, N) and return an array of shape (n, 1).
    :param noise_variance: float.
    :param rng: NumPy rng object.
    :return: Callable that takes in an array of shape (n, N) and returns an array of shape (n, N).
    """
